...
| File | Purpose |
|------|---------|
| `pkg/toolset/yunxiao/toolset.go` | Toolset registration, `compactHiddenTools` map (59 hidden tools across 4 categories), write-tool gating |
| `pkg/toolset/yunxiao/*_tools.go` | 191 MCP tool schemas across 7 domains (170 read-only + 16 write-capable tools gated behind read_only=false) |
