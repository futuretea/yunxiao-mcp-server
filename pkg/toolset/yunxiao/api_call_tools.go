	...
	return []toolset.ServerTool{
		{
			Tool: mcp.NewTool("call_yunxiao_api",
				mcp.WithDescription("Execute a read-only Yunxiao OpenAPI call. Use this as a fallback for endpoints not covered by dedicated tools. Only GET and POST read-only endpoints are supported. Prefer dedicated tools (e.g., search_workitems, list_pipelines) when available, as they provide better parameter validation and error guidance."),
