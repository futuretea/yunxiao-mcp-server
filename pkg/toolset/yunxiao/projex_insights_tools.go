	...
	return []toolset.ServerTool{
		{
			Tool: mcp.NewTool("get_project_risk_dashboard",
				mcp.WithDescription("Get a read-only project risk dashboard with category samples, overdue work items, and optional high-priority/stale sections."),
		...
		},
		{
			Tool: mcp.NewTool("get_project_member_task_status",
				mcp.WithDescription("Get per-member task status for one project with assigned, overdue, and optional status-group sections."),
				mcp.WithString("organizationId", mcp.Required(), mcp.Description("Yunxiao organization ID.")),
				mcp.WithString("id", mcp.Required(), mcp.Description("Project ID.")),
